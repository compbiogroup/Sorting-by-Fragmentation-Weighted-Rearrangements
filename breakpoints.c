...
int is_R_breakpoint(permutation_t *p, int i, int j){
  if (i == p->size+1 || j == 0 || j != i+1) {
      printf("ERRO! is_R_breakpoint(%d, %d)\n", i, j);
      exit(0);
  }

  //in this case, breakpoints at the beginning or the end of the permutation are not considered
  if(i == 0 || j == p->size+1){
    return 0;
  }
...
int is_T_breakpoint(permutation_t *p, int i, int j){
  if (i == p->size+1 || j == 0 || j != i+1) {
      printf("ERRO! is_T_breakpoint(%d, %d)\n", i, j);
      exit(0);
  }
...
int is_RT_breakpoint(permutation_t *p, int i, int j){
  if (i == p->size+1 || j == 0 || j != i+1) {
      printf("ERRO! is_R_breakpoint(%d, %d)\n", i, j);
      exit(0);
  }
...
int is_sig_R_breakpoint(permutation_t *p, int i, int j){
  if (i == p->size+1 || j == 0 || j != i+1) {
      printf("ERRO! is_T_breakpoint(%d, %d)\n", i, j);
      exit(0);
  }
...
int is_sig_RT_breakpoint(permutation_t *p, int i, int j){
  if (i == p->size+1 || j == 0 || j != i+1) {
      printf("ERRO! is_T_breakpoint(%d, %d)\n", i, j);
      exit(0);
  }
...
int nof_R_breakpoints(permutation_t *p) {/*{{{*/
    int i, bp = 0;
    for (i = 0; i <= p->size; i++)
        if (is_R_breakpoint(p, i, i+1))
            bp++;
    return bp;
}/*}}}*/

int nof_T_breakpoints(permutation_t *p) {/*{{{*/
    int i, bp = 0;
    for (i = 0; i <= p->size; i++)
        if (is_T_breakpoint(p, i, i+1))
            bp++;
    return bp;
}/*}}}*/

int nof_RT_breakpoints(permutation_t *p) {/*{{{*/
    int i, bp = 0;
    for (i = 0; i <= p->size; i++)
        if (is_RT_breakpoint(p, i, i+1))
            bp++;
    return bp;
}/*}}}*/


int nof_sig_R_breakpoints(permutation_t *p) {/*{{{*/
    int i, bp = 0;
    for (i = 0; i <= p->size; i++)
        if (is_sig_R_breakpoint(p, i, i+1))
            bp++;
    return bp;
}/*}}}*/

int nof_sig_RT_breakpoints(permutation_t *p) {/*{{{*/
    int i, bp = 0;
    for (i = 0; i <= p->size; i++)
        if (is_sig_RT_breakpoint(p, i, i+1))
            bp++;
    return bp;
}/*}}}*/
...
int elem_is_R_breakpoint(permutation_t *p, int i, int j) {/*{{{*/
    if (i == p->size+1 || j == 0) {
        printf("ERRO! elem_is_R_breakpoint()\n");
        exit(0);
    }
...
int elem_is_T_breakpoint(permutation_t *p, int i, int j) {/*{{{*/
    if (i == p->size+1 || j == 0) {
        printf("ERRO! elem_is_T_breakpoint()\n");
        exit(0);
    }
...
int elem_is_RT_breakpoint(permutation_t *p, int i, int j) {/*{{{*/
    if (i == p->size+1 || j == 0) {
        printf("ERRO! elem_is_R_breakpoint()\n");
        exit(0);
    }
...
int elem_is_sig_R_breakpoint(permutation_t *p, int i, int j) {/*{{{*/
  if (i == p->size+1 || j == 0) {
    printf("ERRO! elem_is_sig_R_breakpoint()\n");
    exit(0);
  }
...
int elem_is_sig_RT_breakpoint(permutation_t *p, int i, int j) {/*{{{*/
  if (i == p->size+1 || j == 0) {
    printf("ERRO! elem_is_sig_R_breakpoint()\n");
    exit(0);
  }
...
int nofb_after_r(permutation_t *p, int i, int j, int nof_b){
  if(elem_is_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
      !elem_is_R_breakpoint(p, p->pi[i-1], p->pi[j])){
    nof_b--;
  }else if(!elem_is_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
          elem_is_R_breakpoint(p, p->pi[i-1], p->pi[j])){
    nof_b++;
  }

  if(elem_is_R_breakpoint(p, p->pi[j], p->pi[j+1]) &&
      !elem_is_R_breakpoint(p, p->pi[i], p->pi[j+1])){
    nof_b--;
  }else if(!elem_is_R_breakpoint(p, p->pi[j], p->pi[j+1]) &&
      elem_is_R_breakpoint(p, p->pi[i], p->pi[j+1])){
    nof_b++;
  }
...
int nofb_after_t(permutation_t *p, int i, int j, int k, int nof_b){
  if(elem_is_T_breakpoint(p, p->pi[i-1], p->pi[i]) &&
      !elem_is_T_breakpoint(p, p->pi[i-1], p->pi[j])){
    nof_b--;
  }else if(!elem_is_T_breakpoint(p, p->pi[i-1], p->pi[i]) &&
          elem_is_T_breakpoint(p, p->pi[i-1], p->pi[j])){
    nof_b++;
  }

  if(elem_is_T_breakpoint(p, p->pi[j-1], p->pi[j]) &&
      !elem_is_T_breakpoint(p, p->pi[k-1], p->pi[i])){
    nof_b--;
  }else if(!elem_is_T_breakpoint(p, p->pi[j-1], p->pi[j]) &&
      elem_is_T_breakpoint(p, p->pi[k-1], p->pi[i])){
    nof_b++;
  }

  if(elem_is_T_breakpoint(p, p->pi[k-1], p->pi[k]) &&
      !elem_is_T_breakpoint(p, p->pi[j-1], p->pi[k])){
    nof_b--;
  }else if(!elem_is_T_breakpoint(p, p->pi[k-1], p->pi[k]) &&
          elem_is_T_breakpoint(p, p->pi[j-1], p->pi[k])){
    nof_b++;
  }
...
int nofb_after_rt(permutation_t *p, char rearr, int i, int j, int k, int nof_b){

  if(rearr == 'r'){
    if(elem_is_RT_breakpoint(p, p->pi[i-1], p->pi[i]) &&
        !elem_is_RT_breakpoint(p, p->pi[i-1], p->pi[j])){
      nof_b--;
    }else if(!elem_is_RT_breakpoint(p, p->pi[i-1], p->pi[i]) &&
            elem_is_RT_breakpoint(p, p->pi[i-1], p->pi[j])){
      nof_b++;
    }

    if(elem_is_RT_breakpoint(p, p->pi[j], p->pi[j+1]) &&
        !elem_is_RT_breakpoint(p, p->pi[i], p->pi[j+1])){
      nof_b--;
    }else if(!elem_is_RT_breakpoint(p, p->pi[j], p->pi[j+1]) &&
        elem_is_RT_breakpoint(p, p->pi[i], p->pi[j+1])){
      nof_b++;
    }
  }else if(rearr == 't'){
    ...
    if(elem_is_RT_breakpoint(p, p->pi[j-1], p->pi[j]) &&
        !elem_is_RT_breakpoint(p, p->pi[k-1], p->pi[i])){
      nof_b--;
    }else if(!elem_is_RT_breakpoint(p, p->pi[j-1], p->pi[j]) &&
        elem_is_RT_breakpoint(p, p->pi[k-1], p->pi[i])){
      nof_b++;
    }

    if(elem_is_RT_breakpoint(p, p->pi[k-1], p->pi[k]) &&
        !elem_is_RT_breakpoint(p, p->pi[j-1], p->pi[k])){
      nof_b--;
    }else if(!elem_is_RT_breakpoint(p, p->pi[k-1], p->pi[k]) &&
            elem_is_RT_breakpoint(p, p->pi[j-1], p->pi[k])){
      nof_b++;
    }
  }
...
int nofb_after_sig_rt(permutation_t *p, char rearr, int i, int j, int k, int nof_b){

  if(rearr == 'r'){
    if(elem_is_sig_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
        !elem_is_sig_R_breakpoint(p, p->pi[i-1], -p->pi[j])){
      nof_b--;
    }else if(!elem_is_sig_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
            elem_is_sig_R_breakpoint(p, p->pi[i-1], -p->pi[j])){
      nof_b++;
    }

    if(elem_is_sig_R_breakpoint(p, p->pi[j], p->pi[j+1]) &&
        !elem_is_sig_R_breakpoint(p, -p->pi[i], p->pi[j+1])){
      nof_b--;
    }else if(!elem_is_sig_R_breakpoint(p, p->pi[j], p->pi[j+1]) &&
        elem_is_sig_R_breakpoint(p, -p->pi[i], p->pi[j+1])){
      nof_b++;
    }
  }else if(rearr == 't'){
    if(elem_is_sig_RT_breakpoint(p, p->pi[i-1], p->pi[i]) &&
        !elem_is_sig_RT_breakpoint(p, p->pi[i-1], p->pi[j])){
      nof_b--;
    }else if(!elem_is_sig_RT_breakpoint(p, p->pi[i-1], p->pi[i]) &&
            elem_is_sig_RT_breakpoint(p, p->pi[i-1], p->pi[j])){
      nof_b++;
    }

    if(elem_is_sig_RT_breakpoint(p, p->pi[j-1], p->pi[j]) &&
        !elem_is_sig_RT_breakpoint(p, p->pi[k-1], p->pi[i])){
      nof_b--;
    }else if(!elem_is_sig_RT_breakpoint(p, p->pi[j-1], p->pi[j]) &&
        elem_is_sig_RT_breakpoint(p, p->pi[k-1], p->pi[i])){
      nof_b++;
    }

    if(elem_is_sig_RT_breakpoint(p, p->pi[k-1], p->pi[k]) &&
        !elem_is_sig_RT_breakpoint(p, p->pi[j-1], p->pi[k])){
      nof_b--;
    }else if(!elem_is_sig_RT_breakpoint(p, p->pi[k-1], p->pi[k]) &&
            elem_is_sig_RT_breakpoint(p, p->pi[j-1], p->pi[k])){
      nof_b++;
    }
  }
...
int nofb_after_sig_r(permutation_t *p, int i, int j, int nof_b){
  if(elem_is_sig_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
      !elem_is_sig_R_breakpoint(p, p->pi[i-1], -p->pi[j])){
    nof_b--;
  }else if(!elem_is_sig_R_breakpoint(p, p->pi[i-1], p->pi[i]) &&
