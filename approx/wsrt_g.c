...
void alg_2SRT_g(permutation_t *p, int verbose, info_t* info) {/*{{{*/
    int i, j, k;
    int best_i, best_j, best_k, best_rearr;
    float best_cost, op_cost;

    int nof_b = nof_sig_RT_breakpoints(p);
    while(nof_b > 0){
      best_cost = -1;
      for(i = 1; i <= p->size; i++){
        ...
        for(j = i; j <= p->size; j++){
          op_cost = (nof_b - nofb_after_sig_rt(p, 'r', i, j, 0, nof_b))/ (fw_r(i, j, p->size) *1.0);
      ...
      for(i = 1; i < p->size; i++){
        ...
        for(j = i+1; j <= p->size; j++){
          ...
          for(k = j+1; k <= p->size+1; k++){
            op_cost = (nof_b - nofb_after_sig_rt(p, 't', i, j, k, nof_b))/ (fw_t(i, j, k, p->size) *1.0);
