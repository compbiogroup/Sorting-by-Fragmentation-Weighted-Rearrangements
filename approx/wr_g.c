...
int first_strip_end(permutation_t *p){
  ...
  while(abs(p->pi[j] - p->pi[j+1]) == 1 && j < p->size){
    j++;
  }
...
void alg_2R_g(permutation_t *p, int verbose, info_t* info) {/*{{{*/
    ...
    int nof_b = nof_R_breakpoints(p);
    while(nof_b > 0){
      best_cost = -1;

      for(i = 1; i < p->size; i++){
        for(j = i+1; j <= p->size; j++){
          op_cost = (nof_b - nofb_after_r(p, i, j, nof_b))/ (fw_r(i, j, p->size) *1.0);
      ...
      if(best_cost > 0){
        ...
        int j = first_strip_end(p);
        if(p->pi[j] == 1){
          int next_j = find_strip_containing(p, p->pi[1] + 1, 0);

          op_reversal(p, next_j, p->size);
          print_verbose(p, verbose, 'r', next_j, p->size, 0);
          info->nmoves++;
          info->weight += fw_r(next_j, p->size, p->size);//i,j,size

        }else if(p->pi[j] == p->size){
          int next_j = find_strip_containing(p, p->pi[1] - 1, 1);
