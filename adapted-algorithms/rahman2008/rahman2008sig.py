...
def reversalsig(permutation,i,j):
  n = len(permutation)
  new = []
  for x in range(0,i):
    new.append(permutation[x])
...
def get_cycles(str_permutation) :
    #cmd = 'python cycle_decomposition.py %s' % str_permutation.replace("-","")
    #cycles = eval(commands.getoutput(cmd))
    #print cycles
    cmd = 'python signed_cycle_decomposition.py %s' % str_permutation
    cycles = eval(commands.getoutput(cmd))
    #print cycles
    return cycles
...
if __name__ == "__main__" :
    ...
    permutation = eval("[%s]" % str_permutation.replace("-",""))
    n = len(permutation)        
    ...
    permutationsig = eval("[%s]" % str_permutation)
    permutationsig = [0] + permutationsig + [n+1]
        ...
        for cycle_index in range(len(cycles)) :
            ...
            if len(cycle) > 3 :
                ...
                if [i,j,k] != [0,0,0] :
                    ...
                    permutationsig = transposition(permutationsig, i, j,k)
                    position    = get_position(permutation)
                    distance    = distance + 1
            ...
            for cycle_index in range(len(cycles)) :
                ...
                if len(cycle) > 2 :
                    ...
                    if [i,j] != [0,0] :
                        ...
                        permutationsig = reversalsig(permutationsig, i, j)
                        position    = get_position(permutation)
                        distance    = distance + 1
                        sequence.append([i,j])
                        cycles.__delitem__(cycle_index)
    ...
    original_permutation_size = len(permutation) -2 #the elementos 0 and n+1 were added in the permutation
    #print len(sequence),
    #print(sequence)
    cost = 0
